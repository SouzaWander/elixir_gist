...
Hooks.UpdateLineNumbers = {
    mounted() {
        ...
        this.el.addEventListener("scroll", () => {
            const lineNubersText = document.querySelector("#line-numbers")
            lineNubersText.scrollTop = this.el.scrollTop
        })
    ...
    updateLineNumbers() {
        const lineNubersText = document.querySelector("#line-numbers");
        if (!lineNubersText) return;

        const lines = this.el.value.split("\n");

        const numbers = lines.map((_, index) => index + 1).join("\n") + "\n"

        lineNubersText.value = numbers
    }
};
